document.addEventListener("DOMContentLoaded", function () {
  ...
  var calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    events: {
      ...
      success: function (result) {
        ...
        result["tasks"].forEach(function (obj) {
          let start = moment(obj.start_date).format();
          let end = moment(obj.end_date).format();
          let duration = moment(end).diff(start, "days") + 1;
          let overdue =
            currentDate >= end ? moment(currentDate).diff(end, "days") + 1 : 0;
          ...
          //? workaround for overdue tasks.
          //? fullcalendar can't show overdue so new event is pushed with overdue data
          if (!obj.finished) {
            if (moment(obj.end_date).format() < currentDate) {
              events.push({
                id: obj.id + "o",
                title: obj.title,
                start:
                  moment(obj.start_date).format("YYYY-MM-DD") ==
                  moment(obj.end_date).format("YYYY-MM-DD")
                    ? moment(obj.start_date).add(overdue, "days").format()
                    : moment(obj.start_date).add(duration, "days").format(),
                end: moment(currentDate).add(1, "days").format(),
                allDay: true,
                editable: false,
                // textColor: moment(obj.start_date).format('YYYY-MM-DD') == moment(obj.end_date).format('YYYY-MM-DD') ? '#f00a0a' : '',
                color: "#f00a0a",
                display:
                  moment(obj.start_date).format("YYYY-MM-DD") ==
                  moment(obj.end_date).format("YYYY-MM-DD")
                ...
                extendedProps: {
                  description: obj.description,
                  startOriginal: obj.start_date,
                  endOriginal: obj.end_date,
                  allDayOriginal: obj.all_day == "0" ? false : true,
                  started: obj.started,
                  finished: obj.finished,
                  overdue: overdue,
                  spaceName: obj.space_name,
    ...
    eventContent: function (arg) {
      let event = arg.event;
      // console.log(arg);
      if (!event.extendedProps.finished && event.extendedProps.overdue) {
        ...
        //* VARIANT2: show progress bar as transparent overlay
        let progress_bar =
          !event.extendedProps.started && start !== end
            ? ""
            : "<div class='row g-0'><div data-progress-id='" +
              event.id +
              "' class='col' style='position:absolute;inset:0; height:100%;background-color:rgba(255,255,255,0.4)'></div></div>";
        customHtml =
          "<div class='position-relative g-0'>" +
          progress_bar +
          "<div class='row g-0'><div data-event-id='" +
          event.id +
          "'class='col-auto position-relative m-0 p-0 px-1 fw-bold" +
          fontColor +
          "'>" +
          event.title +
          "<span class='position-absolute top-0 start-100 translate-middle-y badge bg-light rounded-pill border border-danger text-danger fw-normal'>+" +
          event.extendedProps.overdue +
          "<small>d</small></span></div></div>";
    ...
    eventDidMount: function (info) {
      ...
      //* CONTENT FORMAT TO SHOW IN TOOLTIP
      /* let content = '<div class="card-body p-2" style="min-width:200px">';
      content +=
        !event.extendedProps.finished && event.extendedProps.overdue > 0
          ? '<p class="my-0 py-0 mb-2"><small class="card-title text-danger">Task overdue: ' +
            event.extendedProps.overdue +
            "day(s)</small></p>"
          : "";
      content += '<h5 class="card-title my-0">' + event.title + "</h5>";
      content += event.extendedProps.spaceName
        ? '<p class="my-0 py-0 mb-2"><small class="card-subtitle text-muted">Space: ' +
          event.extendedProps.spaceName +
          "</small></p>"
        : "";
      content += event.extendedProps.description
        ? '<p class="my-0 py-0"><small class="text-muted">Description:</small></p><p class="card-text my-2 mt-0">' +
          event.extendedProps.description +
          "</p>"
        : "";
      content += event.start
        ? '<p class="card-text my-0 text-muted">Start: <span class="float-end">' +
          moment(event.start).format("DD/MM/YYYY, HH:mm") +
          "</span></p>"
        : "";
      content += event.end
        ? '<p class="card-text my-0 text-muted">End: <span class="float-end">' +
          moment(event.end).format("DD/MM/YYYY, HH:mm") +
          "</span></p>"
        : "";
      content += event.extendedProps.finished
        ? '<p class="card-text my-0 text-muted">Finished: <span class="float-end">' +
          moment(event.extendedProps.finished).format("DD/MM/YYYY, HH:mm") +
          "</span></p>"
        : "";
      content += "</div>"; */

      //* SHOW TOOLTIP WHEN MOUSEOVER TASK IN CALENDAR
      /*tippy(info.el, {
        interactive: true,
        // trigger: 'click',
        theme: "light-border",
      ...
      //* ADJUST 'LEFT' PROPERTY TO SHOW ACTUAL STARTED DATE FOR TASKS THAT SPREAD
      ...
      if (progress_bar && calendar.view.type !== "listWeek") {
        // calendar cell width
        let calendar_cell_width =
          info.el.parentElement.parentElement.offsetWidth;
        // task div element width in calendar
        let calendar_task_width = info.el.parentElement.offsetWidth;
        // how many cells in calendar event spreads
        let cells_across =
          Math.ceil(calendar_task_width / calendar_cell_width) - 1;
        // task start date in calendar row
        let task_start_date_in_row = moment(
          info.el.closest("td").getAttribute("data-date")
        ).format("YYYY-MM-DD");
        // task end date in calendar row
        let task_end_date_in_row = moment(task_start_date_in_row)
          .add(cells_across - 1, "days")
          .format("YYYY-MM-DD");
        // console.log(task_end_date_in_row);
        // actual starting date
        let started = moment(event.extendedProps.started).format("YYYY-MM-DD");
        ...
        if (last_date_in_row < started) {
          info.el
            .querySelector("div[data-progress-id='" + event.id + "'")
            .closest("div[data-progress-id='" + event.id + "'")
          ...
          // calculate difference in days between 'task_end_date_in_row' and 'last_date_in_row' date
          let offset_end = moment(last_date_in_row).diff(
            task_end_date_in_row,
            "days"
          );
          ...
          let calc_left =
            offset_start > 0
              ? offset_start * calendar_cell_width + 2 - correction
              : 1;
          // calculate 'width' property value
          let calc_width =
            calendar_task_width -
            (offset_start * calendar_cell_width + 2 - correction) -
            correction;
          ...
          progress_bar.style.left =
            offset_end > 0 ? calc_left - 1 + "px" : calc_left + "px";
          progress_bar.style.width =
            offset_end > 0 ? calc_width - 3 + "px" : calc_width + "px";
        }
      }

      if (calendar.view.type == "listWeek") {
        let status_element = info.el.querySelector("td.fc-list-event-graphic");
        let status;
        if (event.extendedProps.finished) {
          status = '<i class="far fa-check-circle fa-lg text-success"></i>';
        } else if (!event.extendedProps.started) {
          status = '<i class="far fa-clock fa-lg text-muted"></i>';
        } else if (event.extendedProps.started) {
          status = '<i class="far fa-clock fa-lg text-primary"></i>';
        }

        if (event.extendedProps.overdue > 0) {
          status += '<i class="fas fa-clock fa-lg text-danger ps-1"></i>';
        }
...
function populateViewEditFormModal(info) {
  ...
  let start = data.extendedProps.startOriginal
    ? data.extendedProps.startOriginal
    : data.start;
  let end = data.extendedProps.endOriginal
    ? data.extendedProps.endOriginal
    : data.end;
  let allDay = data.extendedProps.overdue
    ? data.extendedProps.allDayOriginal
    : data.allDay;
  // set start date
  $("#editTaskStartDate")
